...
void memory_read_short_unsigned(uint16_t *p, uint16_t result)
{
  if (*p != result)
...
void memory_read_short(int16_t *p, int16_t result)
{
  if (*p != result)
...
{
  uint8_t *p = (char*)&src->b;

  memory_swl(src, val);

  if (p[0] != a || p[1] != b || p[2] != c || p[3] != d)
    FAIL("swl: 0x%x != 0x%02x%02x%02x%02x\n", val, p[0], p[1], p[2], p[3]);
  else
    PASS("swl: 0x%x != 0x%02x%02x%02x%02x\n", val, p[0], p[1], p[2], p[3]);
}
...
void memory_run(void)
{
  ...
  memory_read_short_unsigned((uint16_t*)(&memory_vals[0]), (uint16_t)0xfffe);
  memory_read_short_unsigned((uint16_t*)(&memory_vals[6]), (uint16_t)0x1dff);
  ...
  tmp = misalign2;
  memory_test_swl(&tmp, 0xff001234, 0xff, 0x00, 0x12, 0x34);
}
